...
module.exports = async (cl, args, msg, sendmsg, hercai, Player) => {
  const sub = args[0] || null;
  const sub2 = args[1] || null;
  if (sub) {var subcommand = sub.toLowerCase();}
  if (sub2) {var subcommand2 = sub2.toLowerCase();}
  ...
  if (subcommand === "animal") {
    ...
    if (!subcommand2)
      return sendmsg("Pick one: IPv4, IPv6, or MAC");
    if (subcommand2 === "ipv4") {
